...
void XYSmoothCurveDock::initGeneralTab() {
	...
	const int xIndex = plot->coordinateSystem(m_curve->coordinateSystemIndex())->index(CartesianCoordinateSystem::Direction::X);
...
void XYSmoothCurveDock::recalculateClicked() {
	...
	Q_EMIT info(i18n("Smoothing status: %1", m_smoothCurve->smoothResult().status));
