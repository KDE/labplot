...
bool raiseDock(MainWin* w, AspectType t, QStackedWidget* parent) {
    ...
    const bool generated = w->generateDock<T>(t, parent);
    auto dock = w->dock<T>(t);
    if (generated) {
...
void raiseDockConnect(MainWin* w, AspectType t, QStatusBar* statusBar, QStackedWidget* parent) {
    if (raiseDock<T>(w, t, parent))
        QObject::connect(w->dock<T>(t), &T::info, [=](const QString& text) {
...
void GuiObserver::selectedAspectsChanged(QList<AbstractAspect*>& selectedAspects) const {
	...
	case AspectType::Spreadsheet:
        ...
        auto* dock = static_cast<SpreadsheetDock*>(m_mainWindow->m_dock.value(type));
...
void GuiObserver::hiddenAspectSelected(const AbstractAspect* aspect) const {
    ...
    switch (parent->type()) { // cast the enum to turn off warnings about unhandled cases
    case AspectType::Axis:
        if (m_mainWindow->generateDock<AxisDock>(AspectType::Axis, m_mainWindow->stackedWidget)) {
            m_mainWindow->stackedWidget->addWidget(m_mainWindow->dock<AxisDock>(AspectType::Axis));
		}
        m_mainWindow->dock<AxisDock>(AspectType::Axis)->activateTitleTab();
		break;
    case AspectType::CartesianPlot:
        if (m_mainWindow->generateDock<CartesianPlotDock>(AspectType::CartesianPlot, m_mainWindow->stackedWidget)) {
            m_mainWindow->stackedWidget->addWidget(m_mainWindow->dock<CartesianPlotDock>(AspectType::CartesianPlot));
        }
        m_mainWindow->dock<CartesianPlotDock>(AspectType::CartesianPlot)->activateTitleTab();
		break;
    case AspectType::CartesianPlotLegend:
        if (m_mainWindow->generateDock<CartesianPlotLegendDock>(AspectType::CartesianPlotLegend, m_mainWindow->stackedWidget)) {
            m_mainWindow->stackedWidget->addWidget(m_mainWindow->dock<CartesianPlotLegendDock>(AspectType::CartesianPlotLegend));
        }
        m_mainWindow->dock<CartesianPlotLegendDock>(AspectType::CartesianPlotLegend)->activateTitleTab();
		break;
	default:
		break;
	}
