...
void GuiObserver::selectedAspectsChanged(QList<AbstractAspect*>& selectedAspects) const {
	...
	if (className == "Spreadsheet") {
		...
		if (!m_mainWindow->plot3dDock){
			m_mainWindow->plot3dDock = new Plot3DDock(m_mainWindow->stackedWidget);
			m_mainWindow->stackedWidget->addWidget(m_mainWindow->plot3dDock);
		}
		...
		if (!m_mainWindow->axes3dDock){
			...
			m_mainWindow->stackedWidget->addWidget(m_mainWindow->axes3dDock);
		}
		...
		if (!m_mainWindow->surface3dDock){
			...
			m_mainWindow->stackedWidget->addWidget(m_mainWindow->surface3dDock);
		}
		...
		m_mainWindow->surface3dDock->setSurfaces(list);

		m_mainWindow->stackedWidget->setCurrentWidget(m_mainWindow->surface3dDock);
	}else if(className == "Curve3D"){
		if (!m_mainWindow->curve3dDock){
			m_mainWindow->curve3dDock = new Curve3DDock(m_mainWindow->stackedWidget);
			m_mainWindow->stackedWidget->addWidget(m_mainWindow->curve3dDock);
		}
		...
		m_mainWindow->stackedWidget->setCurrentWidget(m_mainWindow->curve3dDock);
