...
#include <KConfig>
 ...
 /*!
  \class ThemeHandler
  \brief Provides a widget with buttons for saving and loading of themes.
...
ThemeHandler::ThemeHandler(QWidget *parent, ClassName name): QWidget(parent){
...
void ThemeHandler::retranslateUi(){
    tbLoad->setToolTip(i18n("Load properties from a theme"));
    tbSave->setToolTip(i18n("Save current properties as a theme"));
...
void ThemeHandler::loadMenuSelected(QAction* action) {
...
/*!
 * Is called when the current properties are going to be saved as a new theme.
...
void ThemeHandler::saveNewSelected(const QString& filename) {
    ...
    //we have at least one saved theme now -> enable the load button
...
/*!
 * Is called when the current properties are going to be saved in an already available theme.
 * Emits \c saveConfigRequested, the receiver of the signal has to config.sync().
 */
void ThemeHandler::saveMenuSelected(QAction* action) {
    KConfig config(action->data().toString()+'/'+action->text(), KConfig::SimpleConfig);
    emit (saveConfigRequested(config));

    emit info( i18n("Theme \"%1\" was saved.", action->text()) );
}
