...
*/
#include "MDFOptionsWidget.h"
#include "ImportFileWidget.h"
#include "backend/datasources/filters/MDFFilter.h"
#include "backend/lib/macros.h"

/*!
   \class MDFOptionsWidget
   \brief Widget providing options for the import of MDF data

   \ingroup kdefrontend
*/
MDFOptionsWidget::MDFOptionsWidget(QWidget* parent, ImportFileWidget* fileWidget)
	: QWidget(parent)
    ...
    connect(ui.twContent, &QTreeWidget::itemSelectionChanged, this, &MDFOptionsWidget::hdf5TreeWidgetSelectionChanged);
	connect(ui.bRefreshPreview, &QPushButton::clicked, fileWidget, &ImportFileWidget::refreshPreview);
}

void MDFOptionsWidget::clear() {
	ui.twContent->clear();
	ui.twPreview->clear();
}

void MDFOptionsWidget::updateContent(MDFFilter* filter, const QString& fileName) {
	ui.twContent->clear();
...
*/
void MDFOptionsWidget::hdf5TreeWidgetSelectionChanged() {
	DEBUG(Q_FUNC_INFO);
...
*/
const QStringList MDFOptionsWidget::selectedNames() const {
	DEBUG(Q_FUNC_INFO);
	QStringList names;
