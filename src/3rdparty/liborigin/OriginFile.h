...
public:
    ...
    Origin::SpreadColumn &
    dataset(vector<Origin::SpreadColumn>::size_type ds) const; //!< get dataset ds

    vector<Origin::SpreadSheet>::size_type spreadCount() const; //!< get number of spreadsheets
    Origin::SpreadSheet &
    spread(vector<Origin::SpreadSheet>::size_type s) const; //!< get spreadsheet s
    ...
    vector<Origin::Function>::difference_type
    functionIndex(const string &name) const; //!< get index (or -1) of function named name
