...
void Label::draw(QPainter *p, QPoint pos, QPoint size, int w, int h, double phi) {
...
*/
	//TODO remove textdocument here. use painter only.
	QTextDocument *t = new QTextDocument();
	t->setHtml(m_text);
	t->setDefaultFont(m_textFont);

	p->save();
	p->translate((pos.x()+m_position.x()*size.x())*w, (pos.y()+m_position.y()*size.y())*h);
	// phi : normal angle, rotation : additional rotation
	p->rotate(phi+m_rotation);
...
*/
	...
	p->save();
	p->translate((pos.x()+m_positionX*size.x())*w, (pos.y()+m_positionY*size.y())*h);
	// phi : normal angle, rotation : additional rotation
	p->rotate(phi+m_rotation);
