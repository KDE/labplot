...
QMenu* AbstractPart::createContextMenu(){
	...
	if (m_mdiWindow) {
#ifndef ACTIVATE_SCIDAVIS_SPECIFIC_CODE
        menu->addAction(QIcon(QIcon("document-export-database")), i18n("Export"), this, SIGNAL(exportRequested()));
        menu->addAction(QIcon(QIcon("document-print")), i18n("Print"), this, SIGNAL(printRequested()));
        menu->addAction(QIcon(QIcon("document-print-preview")), i18n("Print Preview"), this, SIGNAL(printPreviewRequested()));
#else
