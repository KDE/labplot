...
// ########################  Data Export  #######################################
// ##############################################################################
bool Spreadsheet::exportToSQLite(const QString& path, const QString& tableName) const {
	if (!path.isEmpty()) {
		QFile file(path);
	...
	// create table
	const int cols = columnCount();
	QString name = tableName.isEmpty() ? this->name() : tableName;
	QString query = QLatin1String("create table ") + name + QLatin1String(" (");
	...
	// create bulk insert statement in batches of 10k rows
	{
		...
		QString insertQuery = QStringLiteral("INSERT INTO '") + name + QStringLiteral("' (");
		for (int i = 0; i < cols; ++i) {
			if (i != 0)
				insertQuery += QLatin1String(", ");
			insertQuery += QLatin1Char('\'') + column(i)->name() + QLatin1Char('\'');
		}
	...
	if (!path.isEmpty()) // don't close the database if we export to an in-memory db (path is empty)
		db.close();
	return true;
}
 ...
 */
int Spreadsheet::maxRowToExport() const {
	int maxRow = -1;
