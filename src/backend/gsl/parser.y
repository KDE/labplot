...
/*	symbol *sym_table;	the symbol table (not used) */
} param;
...
symbol* putsym(const char *sym_name, int sym_type) {
...
symbol* getsym(const char *sym_name) {
	...
	symbol *ptr;
	for (ptr = sym_table; ptr != 0; ptr = (symbol *)ptr->next) {
		/* pdebug("%s ", ptr->name); */
