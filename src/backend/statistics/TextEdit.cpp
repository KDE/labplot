...
void TextEdit::processToolTipTags(QString &text, bool updateTooltips)
{
	...
	while ((searchPos = text.indexOf(startToolTip, searchPos)) != -1) {
		...
		if (dataStartPos != -1 && dataEndPos != -1 &&
			dataStartPos < tooltipEndPos && dataEndPos < tooltipEndPos) {
			dataText = text.mid(dataStartPos + startData.length(),
								dataEndPos - dataStartPos - startData.length());
		} else {
			dataText.clear();
		...
		QString tipText;
		if (tipStartPos != -1 && tipEndPos != -1 &&
			tipStartPos < tooltipEndPos && tipEndPos < tooltipEndPos) {
			tipText = text.mid(tipStartPos + startTip.length(),
							   tipEndPos - tipStartPos - startTip.length());
		} else {
			tipText.clear();
		}
