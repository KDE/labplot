...
void AxisPrivate::retransformRange() {
	...
	case Axis::RangeType::Auto: {
		if (orientation == Axis::Orientation::Horizontal)
			range = q->m_plot->xRange(q->cSystem->index(Direction::X));
		else
			range = q->m_plot->yRange(q->cSystem->index(Direction::Y));
...
void AxisPrivate::retransformTicks() {
	...
	const int xRangeDirection = plot()->xRange(cs->index(Direction::X)).direction();
	const int yRangeDirection = plot()->yRange(cs->index(Direction::Y)).direction();
	...
	const double middleX = plot()->xRange(cs->index(Direction::X)).center();
	const double middleY = plot()->yRange(cs->index(Direction::Y)).center();
	...
	for (int iMajor = 0; iMajor < tmpMajorTicksNumber; iMajor++) {
		...
		if (majorTicksDirection != Axis::noTicks) {
			if (orientation == Axis::Orientation::Horizontal) {
				auto startY = q->plot()->yRange(cs->index(Direction::Y)).start();
				...
				auto startX = q->plot()->xRange(cs->index(Direction::X)).start();
...
void AxisPrivate::retransformMajorGrid() {
	...
	const auto yRange{plot()->yRange(cs->index(Direction::X))};
...
void AxisPrivate::retransformMinorGrid() {
	...
	auto cs = plot()->coordinateSystem(q->coordinateSystemIndex());
	if (orientation == Axis::Orientation::Horizontal) { // horizontal axis
		const Range<double> yRange{plot()->yRange(cs->index(Direction::Y))};

		for (const auto& point : logicalMinorTickPoints)
			lines.append(QLineF(point.x(), yRange.start(), point.x(), yRange.end()));
	} else { // vertical axis
		const Range<double> xRange{plot()->xRange(cs->index(Direction::X))};
...
void AxisPrivate::paint(QPainter* painter, const QStyleOptionGraphicsItem* /*option*/, QWidget*) {
	...
	if (labelsPosition != Axis::LabelsPosition::NoLabels) {
		...
		if (showScaleOffset && tickLabelPoints.size() > 0) {
			...
			auto cs = plot()->coordinateSystem(q->coordinateSystemIndex());
			const qreal middleX = plot()->xRange(cs->index(Direction::X)).center();
			const qreal middleY = plot()->yRange(cs->index(Direction::Y)).center();
			QPointF center(middleX, middleY);
			bool valid = true;
			center = q->cSystem->mapLogicalToScene(center, valid);
