...
class LABPLOT_EXPORT Heatmap : Plot {
#else
...
public:
	...
	virtual bool minMax(const Dimension dim, const Range<int>& indexRange, Range<double>& r, bool includeErrorBars = true) const override;
	virtual bool indicesMinMax(const Dimension dim, double v1, double v2, int& start, int& end) const override;
	virtual double minimum(Dimension dim) const override;
