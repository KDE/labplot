...
QString Heatmap::saveName = QStringLiteral("Heatmap");
...
int Heatmap::Format::index(double value) const {
	return static_cast<int>((value - start) * (end - start) / colors.size());
}
...
HEATMAP_COLUMN_CONNECT(Heatmap, X, x)
HEATMAP_COLUMN_CONNECT(Heatmap, Y, y)
void Heatmap::connectMatrix(const Matrix* matrix) {
	connect(matrix->parentAspect(), &AbstractAspect::childAspectAboutToBeRemoved, this, &Heatmap::matrixAboutToBeRemoved);
...
public:
	...
	void redo() override {
		...
		finalize();
		// emit m_private->q->matrixChanged(m_matrix);
	}
	void undo() override {
		redo();
	}
...
bool Heatmap::minMaxSpreadsheet(const CartesianCoordinateSystem::Dimension dim, const Range<int>& indexRange, Range<double>& r) const {
	switch (dim) {
	case Dimension::X:
		return XYCurve::minMax(xColumn(), yColumn(), XYCurve::ErrorType::NoError, nullptr, nullptr, indexRange, r, false);
	case Dimension::Y:
		return XYCurve::minMax(yColumn(), xColumn(), XYCurve::ErrorType::NoError, nullptr, nullptr, indexRange, r, false);
	}
	return false;
}
...
void HeatmapPrivate::update() {
	...
	case Heatmap::DataSource::Spreadsheet:
		...
		xNumBins = xColumn->rowCount();
		yNumBins = yColumn->rowCount();
	case Heatmap::DataSource::Matrix:
		if (!matrix) {
			DEBUG(Q_FUNC_INFO << ", WARNING: matrix not available");
			return;
		}
		xNumBins = matrix->columnCount();
	...
	const auto xMin = q->minimum(Dimension::X);
	const auto yMin = q->minimum(Dimension::Y);
	...
	QPointF startPoint;
	const double xMinValid = xRangeMin - xBinSize;
	const double xMaxValid = xRangeMax + xBinSize;
	// startPoint: xMin + n*xBinSize >= xMinValid; --> n >= (xMinValid - xMin) / xBinSize
	// round down
	startPoint.setX(xMin + static_cast<int>((xMinValid - xMin) / xBinSize) * xBinSize);
	const int xNumberBinsVisible = (xMaxValid - xMinValid) / xBinSize;
	assert(xNumberBinsVisible >= 0);

	const double yMinValid = yRangeMin - yBinSize;
	const double yMaxValid = yRangeMax + yBinSize;
	const int yNumberBinsVisible = (yMaxValid - yMinValid) / yBinSize;
	startPoint.setY(yMin + static_cast<int>((yMinValid - yMin) / yBinSize) * yBinSize);
	assert(yNumberBinsVisible >= 0);

	std::vector<std::vector<int>> map(xNumberBinsVisible, std::vector<int>(yNumberBinsVisible, 0));
	...
	case Heatmap::DataSource::Spreadsheet: {
		...
		for (int i = 0; i < xNumBins; i++) {
			...
			const int xIndex = static_cast<int>((xVal - xMinValid) * xNumberBinsVisible / (xMaxValid - xMinValid));
			const int yIndex = static_cast<int>((yMaxValid - yVal) * yNumberBinsVisible / (yMaxValid - yMinValid));
	...
	case Heatmap::DataSource::Matrix:
		const double xSteps = (xMax - xMin) * xNumBins;
		const double ySteps = (yMax - yMin) * yNumBins;
		for (int column = 0; column < xNumBins; column++) {
			const double xVal = xMin + xSteps * column;
			const int xIndex = static_cast<int>((xVal - xMinValid) * xNumberBinsVisible / (xMaxValid - xMinValid));
			for (int row = 0; row < yNumBins; row++) {
		...
		for (int yIndex = 0; yIndex < yNumberBinsVisible; yIndex++) {
			for (int xIndex = 0; xIndex < xNumberBinsVisible; xIndex++) {
				...
				if (dataSource == Heatmap::DataSource::Spreadsheet && !drawEmpty && value == 0)
					continue;
				const double xPosStart = startPoint.x() + xIndex * xBinSize;
				const double yPosStart = startPoint.y() + yIndex * yBinSize;
	...
	const auto xEndPoint = startPoint.x() + xNumberBinsVisible * xBinSize;
	const auto yEndPoint = startPoint.y() + yNumberBinsVisible * yBinSize;

	const auto p1 = QPointF(startPoint.x(), startPoint.y());
...
void Heatmap::loadThemeConfig(const KConfig& config) {
	KConfigGroup group = config.group("Heatmap");
