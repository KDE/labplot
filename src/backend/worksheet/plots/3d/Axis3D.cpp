...
BASIC_SHARED_D_READER_IMPL(Axis3D, qsizetype, subSegmentCount, subSegmentCount);
BASIC_SHARED_D_READER_IMPL(Axis3D, qsizetype, segmentCount, segmentCount);
...
STD_SETTER_CMD_IMPL_F_S(Axis3D, SetSegmentCount, qsizetype, segmentCount, updateSegmentCount)
void Axis3D::setSegmentCount(qsizetype count) {
	Q_D(Axis3D);
	if (count != d->segmentCount)
		exec(new Axis3DSetSegmentCountCmd(d, count, ki18n("%1 Axis3D segment count changed")));
}

STD_SETTER_CMD_IMPL_F_S(Axis3D, SetSubSegmentCount, qsizetype, subSegmentCount, updateSubSegmentCount)
void Axis3D::setSubSegmentCount(qsizetype count) {
	Q_D(Axis3D);
	if (count != d->subSegmentCount)
...
void Axis3DPrivate::updateTitle() {
	...
	WorksheetElement* element = static_cast<WorksheetElement*>(q->parentAspect());
	if (element)
		element->changed();
}
...
void Axis3DPrivate::updateSegmentCount() {
	q->m_axis->setSegmentCount(static_cast<int>(segmentCount));
	WorksheetElement* element = static_cast<WorksheetElement*>(q->parentAspect());
	if (element)
		element->changed();
}
...
void Axis3DPrivate::updateSubSegmentCount() {
	q->m_axis->setSubSegmentCount(static_cast<int>(subSegmentCount));
	WorksheetElement* element = static_cast<WorksheetElement*>(q->parentAspect());
	if (element)
		element->changed();
}
